# 🚀 Fzf-lua, Vim-Bookmarks & oil.nvim 사용 가이드

이번 마이그레이션을 통해 도입된 고성능 검색 도구 `fzf-lua`, 북마크 관리 도구 `vim-bookmarks`, 파일 관리 도구 `oil.nvim`의 상세 사용법입니다.
...
## 4. 🛢️ oil.nvim (버퍼 기반 파일 탐색/수정)

`oil.nvim`은 디렉토리를 "수정 가능한 버퍼"처럼 다루는 파일 탐색기입니다.  
현재 설정에서는 숨김 파일이 기본 표시됩니다 (`show_hidden = true`).

### 🔑 기본 단축키 (Keymaps)
| 키 매핑 | 기능 | 설명 |
| :--- | :--- | :--- |
| `<leader>fd` | Oil 열기 | 현재 파일 기준 디렉토리(또는 현재 위치)에서 Oil 버퍼 열기 |

### 💡 기본 사용법

#### 1. 디렉토리 탐색
- `<leader>fd`로 Oil 버퍼를 엽니다.
- 항목에서 `Enter`로 파일/디렉토리를 엽니다.
- `-`로 부모 디렉토리로 이동합니다.

#### 2. 파일/폴더 이름 변경 및 이동
- Oil 버퍼에서 파일명/경로를 직접 수정합니다.
- 수정 후 `:w`를 실행하면 실제 파일시스템에 반영됩니다. (rename/move)

#### 3. 생성/삭제 반영
- 버퍼 목록을 편집(행 추가/삭제)한 뒤 `:w`로 반영할 수 있습니다.
- 변경 전에 어떤 액션이 실행되는지 확인하고 저장하는 습관을 권장합니다.

#### 4. 도움말 확인
- Oil 버퍼 안에서 `g?`를 누르면 사용 가능한 키를 바로 확인할 수 있습니다.
- 버전/설정에 따라 세부 키가 달라질 수 있으므로, 실제 동작은 `g?`를 기준으로 확인하세요.
